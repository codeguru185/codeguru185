...
def process_directory(directory_path, output_path):
    ...
    create_missing_txt_csv(ctl_mapping, txt_mapping, output_path)
...
def create_missing_txt_csv(ctl_mapping, txt_mapping, output_path):
    missing_txt_csv_path = os.path.join(output_path, "missing_txt_mapping.csv")
    with open(missing_txt_csv_path, "w", newline="") as missing_txt_csv:
        csv_writer = csv.writer(missing_txt_csv)
        csv_writer.writerow(["CTL Filename", "CTL Size", "Missing TXT"])
        for ctl_filename, ctl_size in ctl_mapping.items():
            txt_size = txt_mapping.get(ctl_filename, "Missing")
            missing_flag = "Yes" if txt_size == "Missing" else "No"
            csv_writer.writerow([ctl_filename, ctl_size, missing_flag])
